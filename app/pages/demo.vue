  ...
  <div class="ticket-page">
    <div class="container">
      <div class="test-header">
        <h1 class="test-title"><span class="test-icon">📖</span> Демонстрационный билет</h1>
        <div class="test-meta">
      ...
      <div class="question-area">
        <div class="question-head">
          <div class="q-left">
            <button class="q-flag" :class="{ active: flags.has(currentIndex) }" @click="toggleFlag(currentIndex)"
              aria-label="Отметить вопрос" title="Отметить вопрос">⚑</button>
        ...
        <div v-if="current" class="question-card card">
          ...
          <div class="answers">
            <button v-for="(opt, idx) in current.options" :key="opt.id" class="answer" :class="answerClass(idx)"
              :disabled="isAnswered(currentIndex)" @click="selectAnswer(idx)">
...
onMounted(async () => {
  ...
  try {
    ...
    if (saved) {
      ...
      const idx = Number(saved.currentIndex) || 0
      currentIndex.value = Math.min(Math.max(idx, 0), total - 1)
...
async function loadTicket() {
  const q = Array.from({ length: total }).map((_, i) => ({
    id: i + 1,
    text: `Демо-вопрос ${i + 1}. Пример текста — замените на реальный позже.`,
...
const current = computed(() => questions[currentIndex.value] || null)
...
useHead({ title: 'Демо-билет — АвтоТест' })
</script>

<style scoped>
/* переиспользуем визуал теста (скопировано из [id].vue) */
.test-header {
  display: flex;
  align-items: center;
